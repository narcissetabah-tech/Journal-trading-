...
st.title("📈 Dashboard Trading : Analyse & Psychologie")
...
if submitted:
    ...
    st.session_state.journal.append({
        "Date": date, "Actif": actif, "Confirmation": confirmation, 
        "Émotion": emotions, "Notes": notes, "Sens": side,
...
if st.session_state.journal:
    ...
    st.subheader("📋 Historique & Notes")
    # On affiche les notes et confirmations dans le tableau
    ...
    idx = st.selectbox("Détails du trade :", range(len(df)), 
                       format_func=lambda x: f"{df.iloc[x]['Date'].strftime('%d/%m')} - {df.iloc[x]['Actif']} ({df.iloc[x]['PnL ($)']} $)")
    
    # Affichage des notes spécifiques sous le sélecteur
    col_info1, col_info2 = st.columns(2)
    with col_info1:
        st.info(f"**Confirmation :** {df.iloc[idx]['Confirmation']}")
        st.info(f"**Émotion :** {df.iloc[idx]['Émotion']}")
    with col_info2:
        st.warning(f"**Notes :** {df.iloc[idx]['Notes']}")

    col_a, col_b = st.columns(2)
    with col_a:
        if df.iloc[idx]['Img_Avant']: st.image(df.iloc[idx]['Img_Avant'], caption="Analyse AVANT")
    with col_b:
    ...
    st.download_button("📥 Exporter CSV", data=csv, file_name="journal_trading_complet.csv")
else:
    st.info("👋 Votre journal est prêt. Ajoutez un trade pour commencer l'analyse.")
